...
// Генерируем случайные фото с Picsum
...
export default function Home() {
  ...
  // Генерируем случайные фото при загрузке
  const refs: RefItem[] = useMemo(() => {
    return Array.from({ length: TOTAL }).map((_, i) => ({
      id: `ref-${i + 1}`,
      title: `Photo #${i + 1}`,
      subtitle: "Swipe to rate",
      url: getRandomImage(i),
    }));
  }, []);
                    ...
                    {likedList.length === 0 ? (
                      <div className="rounded-xl border border-white/10 bg-black/20 p-5 text-center">
                        ...
                        <div className="mt-1 text-xs text-white/60">
                          Swipe right or tap ❤️ to save photos here.
                        </div>
                      </div>
                    ) : (
