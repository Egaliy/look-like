...
// Тестовые данные с разнообразными изображениями
function makeTestRefs(total = TOTAL): RefItem[] {
  const categories = [
    "Web Design",
    "Mobile App",
    "Landing Page",
  ...
  return Array.from({ length: total }).map((_, i) => {
    ...
    return {
      id: `test-${id}`,
      title: `${category} Reference #${id}`,
      subtitle: "Swipe to rate",
      // Используем разные seeds для разнообразия
      url: `https://picsum.photos/seed/design-${id}/1200/900`,
    };
  });
}

export default function TestPage() {
  const refs = useMemo(() => makeTestRefs(TOTAL), []);
                        ...
                        <Card className="h-full overflow-hidden rounded-[28px] border-white/10 bg-white/5 shadow-2xl">
                          <CardContent className="flex h-full flex-col items-center justify-center p-8 text-center">
                            ...
                            <div className="mt-6 flex w-full gap-2">
                              ...
                              <Button
                                variant="secondary"
                                className="w-full rounded-xl bg-white/5 text-white/80 hover:bg-white/10"
                                onClick={() => {
                                  setIndex(0);
                                  setLikedIds(new Set());
                                  setHistory([]);
